...
$ads_result = mysqli_query($conn,"SELECT * FROM advertisements ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>All Advertisements | Craftzon</title>
<link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css" rel="stylesheet">
<link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css' rel='stylesheet'>
<style>
body{background:#fdf6f0;font-family:'Roboto',sans-serif;}
.container{max-width:1100px;margin:40px auto;}
h1{text-align:center;color:#581845;margin-bottom:30px;}
.grid{display:flex;flex-wrap:wrap;gap:20px;justify-content:center;}
.card{flex:1 1 280px;background:#fff;border-radius:10px;padding:15px;
...
<div class="mc" onclick="if(<?php echo isset($_SESSION['users_id']) ? $_SESSION['users_id'] : 0; ?> == 0) { Swal.fire({title: 'Login Required', text: 'Please login first!', icon: 'warning', showCancelButton: true, confirmButtonText: 'Login Now'}).then((result) => { if(result.isConfirmed) { window.location.href = 'logincraft.php'; } }); } else { document.getElementById('ordersForm').submit(); }" 
  ...
  My Orders
            ...
            <div class="mc active"  onclick="window.location='view_ads.php'" style="cursor:pointer; color:white; font-weight:400;">view advritsment</div>
    ...
    <div class="grid">
        <?php if(mysqli_num_rows($ads_result)>0){
            ...
            <div class="card">
                ...
                <p><strong>Contact:</strong> <?php echo $row['user_email']; ?></p>
            </div>
        <?php }} else { echo "<p>No products advertised yet.</p>"; } ?>
    </div>
	<br><div class="section text-center">
